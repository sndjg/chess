...
- pre-commit hook(`.git/hooks/pre-commit`, 로컬 전용 — clone에 안 딸려옴): ruff check + format --check + pytest(e2e 제외)를 커밋 전에 강제.
...
- `chess_rl.viz.server.create_app(games_dir, extra_policies=None, checkpoint_dir="checkpoints/online_value")`가 FastAPI 앱을 만든다:
  - 리플레이: `GET /`(화면), `GET /api/games`(목록), `GET /api/games/{id}`(moves+result+fens)
  - 인터랙티브 플레이(**사람 수와 AI 응수가 2단계로 분리** — AI 탐색이 수 초 걸리는 동안 사람 수가 먼저 렌더링되게): `GET /play`(화면), `POST /api/play/new`(정책+사람 색 지정해 세션 생성, 사람이 흑이면 AI가 먼저 둠), `POST /api/play/{id}/move`(사람 수만 적용하고 즉시 반환 — 불법이면 400, 프로모션 미지정 시 자동 퀸), `POST /api/play/{id}/ai-move`(AI 응수 계산·적용 — 사람 차례면 400), `GET /api/play/{id}`(상태 재조회), `GET /api/comparison`(계보 간 비교 상태), `GET /api/logs`(최근 서버 로그 + `games_trained`).
  - 세션은 프로세스 메모리에만 보관(`chess_rl/viz/play_session.py`, 수는 UCI/SAN 병행 기록). 게임이 끝나면 자동으로 `games/play_<session_id>.json`에 저장되고, **학습은 백그라운드 스레드로** 진행(응답의 `training`은 항상 None — loss/소요시간은 로그 패널의 `[train]` 라인으로).
  - 상대 정책(`POLICY_PROVIDERS`): `random`(매 게임 새 RandomPolicy), `learning`(매 게임 `learning_policy.new_inference_handle()` — canonical 모델의 독립 복사본, canonical이 학습으로 갱신되면 다음 수 계산 전에 lazy refresh. 동시성 설계는 `online_value_policy.py` 모듈 docstring '동시성' 절). `NetworkPolicy`는 구현만 있고 UI에 안 붙임.
  - `learning`일 때 `/ai-move` 응답 필드: `ai_candidate_moves`(**수 선택과 같은 MCTS 탐색**의 root 통계 — `{move, value: Q, visits}`, 방문 횟수 내림차순), `fen_before_ai_move`, `value_after_ai_move`(백 관점), `games_trained`. `/move` 응답엔 `value_after_human_move`.
  - `extra_policies`로 테스트용 정책 주입(`POLICY_PROVIDERS`가 지역 변수라 monkeypatch 불가), `checkpoint_dir`로 테스트가 실제 checkpoints/를 오염시키지 않게 격리.
  - 프론트 파일은 `Cache-Control: no-cache`로 서빙 — 프론트가 API와 함께 바뀌므로 캐시된 예전 JS가 새 API와 어긋나 조용히 멈추는 사고 방지(실제 발생 사례 있음).
- 프론트는 외부 CDN 없이 vanilla HTML/CSS/JS(`chess_rl/viz/static/`)로, 보드 렌더링 로직은 `board.js`에 공용화(리플레이의 `app.js`, 플레이의 `play.js`가 공유). 플레이 화면: 왼쪽 실제 대국(수는 SAN으로 표시, 하단에 기보 목록), 오른쪽 "AI 생각 스냅샷"(직전 탐색의 후보 수 화살표 — **방문 횟수가 많을수록 진하게**, 가장 진한 화살표 = 실제 둔 수). 그 아래 value 추이 차트, 계보 간 비교 패널(frontier 산점도, 축은 누적 학습 epoch — 없으면 판 수 fallback. 진행 곡선은 산점도와 정보가 중복돼 제거), 서버 로그 패널(레벨 필터 드롭다운, 3초 폴링 — `games_trained` 카운터도 이 폴링으로 실시간 갱신). 프로모션은 기물 선택 버튼(Q/R/B/N), 색 선택에 "랜덤" 있음.
- 설치: `pip install -e ".[dev,viz]"`. 실행: `PYTHONUNBUFFERED=1 conda run --no-capture-output -n chess python -u scripts/serve_viz.py --host 0.0.0.0 --port 8090` (unbuffered가 아니면 로그가 파일로 리다이렉트될 때 실시간으로 안 찍힘).
- `--host 0.0.0.0`은 Tailscale 등 다른 기기 접근용 — 노트북이 연결된 다른 네트워크에도 노출되니 주의. 접속 주소는 `tailscale ip -4`로 확인한 IP:포트.
...
- `chess_rl/eval/arena.py`: `play_match(model_a, model_b, num_games=100, selector_a/selector_b=..., ...)`가 색 50/50 교대로 대국시켜 승/패/무 집계 — 3회 동형반복/50수(클레임형 무승부)도 즉시 무승부로 판정(zobrist 카운팅, 죽은 국면이 질질 끌리는 낭비 방지), max_moves 내 미종료도 무승부 처리. `find_new_frontier(new_model, old_checkpoints, start_idx, ..., on_match=콜백)`가 새 체크포인트가 (보통 다른 family의) 과거 체크포인트 중 어디까지 이기는지, 직전 frontier에서 시작해 이기면 위로/지면 아래로 걸어가며 추적(전수/이분 탐색 아님). 매치별 진행은 `on_match`로 즉시 통지.
- **수 선택 전략은 `mcts.search.MOVE_SELECTORS`에 병치**: `visits`(기본, 방문 횟수)와 `q_among_visited`(충분히 방문된 수 중 Q argmax). `OnlineValuePolicy(move_selector=...)`/`play_match(selector_a/b=...)`로 교체·비교 가능 — 첫 비교 실험은 유의하지 않았음(`docs/IDEAS.md` 참고).
- **실시간 비교 시각화(`/play` 화면)**: 새 checkpoint마다 백그라운드에서 가장 최근에 시작된 *다른* family를 상대로 `find_new_frontier`를 돌린다(100판, mcts_simulations=50 — 200은 매치당 ~13분이라 실측 후 하향). 비교 진행 중에 새 checkpoint가 나오면 **우선순위 대기열**(최신 우선 LIFO, 버리지 않음)에 쌓였다가 유휴 시간에 순서대로 처리. 결과는 `comparison_state`에 매치 단위로 즉시 반영되고 `/play`가 5초 폴링. 학습이 겹치면 진행 중인 학습이 **조기 중단 후 진행분 병합** 뒤 새 학습에 양보(`online_value_policy.py` 동시성 절).
...
- [x] engine: board encoding (13,8,8 plane — 기물 12 + 차례 1; 초기 12-plane은 차례 정보가 없어 value 학습에 결함이 있었음), 고정 action space (64x64 + underpromotion)
- [x] model: policy+value ResNet + `MaterialBlendedPolicyValueNet`(value를 신경망과 기물 점수 휴리스틱의 가중합으로 내는 wrapper — 재료 점수는 입력 plane에서 직접 계산)
...
- [x] OnlineValuePolicy: 판마다 실제 결과로 policy+value head를 함께 학습하는 재미용 AI, viz에 화살표/게이지/차트 시각화 (재현성 의도적 포기). select_move는 MCTS 방문분포 기반(deterministic=사람 대국용 argmax / stochastic=평가 대국용 샘플링), 학습은 replay buffer에서 샘플링한 배치로, policy는 value-delta 가중 방식(REINFORCE는 고정 배치 다스텝에서 발산해 폐기 — `docs/IDEAS.md` 'value-delta 가중 policy 학습' 참고).
